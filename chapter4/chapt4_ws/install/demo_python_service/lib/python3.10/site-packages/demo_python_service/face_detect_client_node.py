...
class FaceDetectorClient(Node):
    ...
    def send_request(self):
        ...
        request.image = self.bridge.cv2_to_imgmsg(self.image)#opencv转换成ros
        # 3.发送并 spin 等待服务处理完成
        future = self.client.call_async(request)#创建服务请求，并异步返回结果
        #现在的future并没有包含响应结果，需要等待服务端完成后才会把结果放到future
        # rclpy.spin_until_future_complete(self, future)#等待服务端返回相应
