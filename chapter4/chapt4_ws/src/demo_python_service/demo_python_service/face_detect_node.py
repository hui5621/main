...
class FaceDetectorionNode(Node):
    ...
    def detect_face_callback(self, request, response):
        if request.image.data:
            cv_image = self.bridge.imgmsg_to_cv2(request.image)#ros转成opencv
        else:
            cv_image = cv2.imread(self.defaut_image_path)#cv_image已经是openc格式的数据
        ...
        response.number = len(face_locations)#人脸个数
